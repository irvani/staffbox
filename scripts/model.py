...
A = dict(
  price=695,            # G subscription $/unit/mo incl. a monthly workflow-review hour (Hadi 23 Sep); founding 100 pay 595 for life
  ...
  sub_cogs=130,         # G $45 tooling/warranty + ~$85 loaded hour for the monthly workflow review
  founding_price=595,   # first 100 units, for life
  churn=0.02,           # G monthly
  partner_share=0.20,   # G blended VAR share across tiers 15/20/30%, paid on every unit incl. Leapfrog
  free_units=100,       # Hadi 23 Sep 2026: first 100 Mac minis given away (no onboarding fee)
)
def run(name, adds, opex, price=None, paid_share=1.0, cogs=None):   # paid_share: fraction of units via cash rev-share channels (0 for Leapfrog-only)
  p = price or A['price']; c = A['sub_cogs'] if cogs is None else cogs; act=0.0; rows=[]; cum=0; shipped=0
  for m in range(1,13):
    a = adds(m); act = act*(1-A['churn']) + a
    founding = min(act, A['free_units'])   # founding units still live, approx
    mrr = act*p - founding*(p-A['founding_price']) if p==A['price'] else act*p
    sub_gp = mrr - act*c
    free = max(0, min(a, A['free_units']-shipped)); shipped += a
    onb_gp = a*(-A['hw_cost']-A['install_cost']) + (a-free)*A['onboard_fee']
    partner = paid_share*mrr*A['partner_share']   # G ~half of units via paying channels; Leapfrog is paid in equity
...
S=[]
S.append(run("Leapfrog only", lambda m: [4,6,8,10,12,12,12,12,12,12,10,10][m-1], opex_base))
...
S.append(run("$2M path @ $595, no review hour", lambda m: [4,8,16,28,48,68,88,104,124,136,144,152][m-1], opex_push, 595, cogs=45))
...
solve={f"${p}":dict(base=need(p,100000),push=need(p,137500)) for p in (595,695,795,995)}
json.dump(dict(assump=A,scen=S,solve=solve),open('resident/model.json','w'),indent=1)
