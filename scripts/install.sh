...
echo "4/5 profile $PROFILE"; hermes profile create "$PROFILE" --clone --description "Staffbox unit: company worker on local models" 2>/dev/null || true
P=~/.hermes/profiles/$PROFILE; cp "$HERE/profile/SOUL.md.example" "$P/SOUL.md"
python3 - "$P/config.yaml" "$MODEL" <<'PY'
import sys,re; p,model=sys.argv[1],sys.argv[2]; s=open(p).read()
s=re.sub(r"(?m)^(\s*provider:\s*).*$", r"\1ollama", s, count=1)
...
open(p,"w").write(s)
PY
echo "5/5 vault"; mkdir -p ~/staffbox/vault/sops; cp -n "$HERE/profile/vault/"*.md ~/staffbox/vault/ 2>/dev/null || true
